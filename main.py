...
VOLUME_BASELINE_MINUTES = env_int("VOLUME_BASELINE_MINUTES", 30)
MIN_VOLUME_BASELINE_MINUTES = env_int("MIN_VOLUME_BASELINE_MINUTES", 3)
...
class Detector:
    def __init__(self, telegram: Telegram):
        ...
        # Per-symbol calendar-minute quote volume buckets used to calculate Volume Spike.
        self.minute_volumes: Dict[str, Deque[List[float]]] = defaultdict(
            lambda: deque(maxlen=VOLUME_BASELINE_MINUTES + 2)
        )
    ...
    def update_minute_volume(self, key: str, timestamp_ms: int, quote_value: float) -> None:
        """Aggregate quote volume into calendar-minute buckets."""
        minute_id = timestamp_ms // 60_000
        buckets = self.minute_volumes[key]
        if buckets and int(buckets[-1][0]) == minute_id:
            buckets[-1][1] += quote_value
        else:
            buckets.append([float(minute_id), quote_value])

    def volume_spike(self, key: str, current_volume: float):
        """
        Compare the current rolling-window volume with the average of completed
        prior minute buckets. Returns (average, multiplier, percent_change, samples).
        """
        buckets = self.minute_volumes[key]
        current_minute = int(time.time() * 1000) // 60_000
        completed = [
            float(volume)
            for minute_id, volume in buckets
            if int(minute_id) < current_minute and volume > 0
        ][-VOLUME_BASELINE_MINUTES:]

        samples = len(completed)
        if samples < MIN_VOLUME_BASELINE_MINUTES:
            return 0.0, 0.0, 0.0, samples

        average = sum(completed) / samples
        if average <= 0:
            return 0.0, 0.0, 0.0, samples

        multiplier = current_volume / average
        percent_change = (current_volume - average) / average * 100
        return average, multiplier, percent_change, samples


    async def fetch_official_candles(self, exchange: str, symbol: str):
        """
        Return chronological 1m candles as dictionaries:
        ts, open, high, low, close, base_volume, quote_volume.
        Data comes from the exchange's official candle endpoint.
        """
        cache_key = self.key(exchange, symbol)
        now = time.time()
        ...
        try:
            if exchange == "gateio":
                ...
                params = {"currency_pair": symbol, "interval": "1m", "limit": CANDLE_LIMIT}
                ...
                params = {"symbol": symbol, "interval": "1m", "limit": CANDLE_LIMIT}
                async with session.get(url, params=params) as r:
                    r.raise_for_status()
                    rows = await r.json()
                # MEXC: [openTime, open, high, low, close, baseVolume, closeTime, quoteVolume, ...]
                for x in rows:
                    candles.append({
                        "ts": int(x[0]),
                        "open": float(x[1]),
                ...
                rows = payload.get("data", [])[:CANDLE_LIMIT]
                # KuCoin classic spot: [time, open, close, high, low, baseVolume, turnover]
                for x in rows:
                    candles.append({
                        "ts": int(x[0]) * 1000,
                        "open": float(x[1]),
            ...
            log.warning("%s %s official candles failed: %s", exchange, symbol, exc)
            return []

    @staticmethod
    def official_candle_stats(candles):
        if len(candles) < MIN_VOLUME_BASELINE_MINUTES + 1:
            return None
        ...
        for candle in candles:
            candle_minute = candle["ts"] // 60_000
            if candle_minute == now_minute:
                current = candle
            elif candle_minute < now_minute:
                completed.append(candle)

        if current is None:
            # Some exchanges return the latest still-open candle with slight timestamp delay.
            current = candles[-1]
            completed = candles[:-1]

        baseline = completed[-VOLUME_BASELINE_MINUTES:]
        if len(baseline) < MIN_VOLUME_BASELINE_MINUTES:
            return None
        ...
        spike = current_volume / avg_quote_volume
        volume_change = (spike - 1) * 100
        change_1m = ((current["close"] / current["open"]) - 1) * 100 if current["open"] else 0.0

        all_for_momentum = completed + [current]
        def momentum(minutes):
            sample = all_for_momentum[-minutes:]
        ...
        return {
            ...
            "change_1m": change_1m,
            "change_5m": momentum(5),
            "change_15m": momentum(15),
        }
    ...
    async def add_trade(self, trade: Trade) -> None:
        ...
        async with self.lock:
            self.update_minute_volume(key, trade.timestamp_ms, trade.quote_value)
            ...
            def window_stats(seconds: int):
                cutoff_ms = now_ms - seconds * 1000
                items = [t for t in window if t.timestamp_ms >= cutoff_ms]
                if len(items) < 2:
                    return 0.0, 0.0, 0.0, 0.0, items
                first = items[0].price
                last = items[-1].price
                change_pct = ((last / first) - 1) * 100 if first > 0 else 0.0
                volume = sum(t.quote_value for t in items)
                buy_volume = sum(t.quote_value for t in items if t.side == "buy")
                buy_ratio = (buy_volume / volume * 100) if volume > 0 else 0.0
                return change_pct, volume, buy_volume, buy_ratio, items

            change_1m, total, buys, buy_pct, trades_1m = window_stats(WINDOW_SECONDS)
            change_5m, volume_5m, buys_5m, buy_pct_5m, trades_5m = window_stats(300)
            change_15m, volume_15m, buys_15m, buy_pct_15m, trades_15m = window_stats(900)

            if change_1m < MIN_CHANGE_1M:
                return
            ...
            # Validate price and volume against official 1-minute candles
            ...
            change_1m = official["change_1m"]
            change_5m = official["change_5m"]
            change_15m = official["change_15m"]
            total = official["current_quote_volume"]
            avg_minute_volume = official["avg_quote_volume"]
            volume_spike_x = official["volume_spike"]
            volume_change_pct = official["volume_change"]
            baseline_samples = official["baseline_count"]
            ...
            if total < MIN_VOLUME_1M_USDT:
                return
            if avg_minute_volume < MIN_AVERAGE_VOLUME_USDT:
                return
            ...
            max_trade_value = max((t.quote_value for t in trades_1m), default=0.0)
            ...
            price_points = min(max(change_1m / 5.0 * 25, 0), 25)
            score += price_points
            if change_1m >= 1.0:
                reasons.append("✅ اختراق سعري سريع")

            if baseline_samples >= MIN_VOLUME_BASELINE_MINUTES and volume_spike_x > 0:
            ...
            baseline_text = (
                f"📈 ارتفاع الفوليوم: <b>{volume_change_pct:+,.0f}%</b>\n"
                f"🔥 Volume Spike: <b>{volume_spike_x:.2f}x</b>\n"
                f"📏 متوسط الدقيقة: <b>{avg_minute_volume:,.0f} USDT</b> ({baseline_samples} دقائق)\n"
                if baseline_samples >= MIN_VOLUME_BASELINE_MINUTES and avg_minute_volume > 0
                else f"📈 ارتفاع الفوليوم: <b>جارٍ بناء المتوسط</b> "
                     f"({baseline_samples}/{MIN_VOLUME_BASELINE_MINUTES} دقائق)\n"
            )
            ...
            message = (
                ...
                f"• الصفقات المستلمة 1m: <b>{len(trades_1m):,}</b>\n"
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        if STARTUP_MESSAGE:
            await telegram.send(
                ...
                f"متوسط الفوليوم: <b>آخر {VOLUME_BASELINE_MINUTES} شمعة 1m رسمية</b>\n"
                f"أقل متوسط دقيقة: <b>{MIN_AVERAGE_VOLUME_USDT:,.0f} USDT</b>\n"
                f"أقل Volume Spike: <b>{MIN_VOLUME_SPIKE:.2f}x</b>"
            )
