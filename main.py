...
MIN_CHANGE_1M = env_float("MIN_CHANGE_1M", 0.30)
...
MIN_TRADES_1M = env_int("MIN_TRADES_1M", 5)
MAX_WS_START_DELAY_SECONDS = env_int("MAX_WS_START_DELAY_SECONDS", 120)
CANDLE_TIMEFRAME = os.getenv("CANDLE_TIMEFRAME", "1m").strip().lower()
...
class Detector:
    ...
    async def fetch_official_candles(self, exchange: str, symbol: str):
        ...
        if CANDLE_TIMEFRAME != "1m":
            raise ValueError("This version supports CANDLE_TIMEFRAME=1m only")
        ...
        try:
            if exchange == "gateio":
                ...
                async with session.get(url, params=params) as r:
                    if r.status == 400 and MEXC_SKIP_INVALID_KLINES:
                        ...
                        if cache_key not in self.logged_invalid_candle_symbols:
                            body = await r.text()
                            log.info(
                                "MEXC skipped unsupported kline symbol %s: HTTP 400 %s",
                                symbol,
                                body[:200],
                            )
                ...
                params = {"symbol": symbol, "granularity": "1min", "limit": str(CANDLE_LIMIT)}
                async with session.get(url, params=params) as r:
                    r.raise_for_status()
                    payload = await r.json()
            ...
            if cache_key not in self.invalid_candle_symbols:
                log.warning("%s %s official 1m candles failed: %s", exchange, symbol, exc)
            return []


    @staticmethod
    def official_candle_stats(candles):
        if len(candles) < MIN_VOLUME_BASELINE_CANDLES + 1:
            return None
        ...
        for candle in candles:
            if candle["ts"] == current_candle_start:
                current = candle
            elif candle["ts"] < current_candle_start:
                completed.append(candle)

        if current is None:
            return None

        baseline = completed[-VOLUME_BASELINE_CANDLES:]
        if len(baseline) < MIN_VOLUME_BASELINE_CANDLES:
            return None
        ...
        elapsed_fraction = min(elapsed_seconds / 60, 1.0)
    ...
    async def add_trade(self, trade: Trade) -> None:
        ...
        now_ms = int(time.time() * 1000)
        retention_seconds = max(60, WINDOW_SECONDS)
        ...
        async with self.lock:
            ...
            current_candle_start_ms = (now_ms // 60_000) * 60_000
            accumulator = self.hour_accumulators.get(key)
            if accumulator is None or accumulator.hour_start_ms != current_candle_start_ms:
                accumulator = HourAccumulator(hour_start_ms=current_candle_start_ms)
                self.hour_accumulators[key] = accumulator

            # Ignore delayed trades belonging to an older hour.
            if trade.timestamp_ms >= current_candle_start_ms:
            ...
            ws_start_delay_seconds = (
                (accumulator.first_trade_ms - current_candle_start_ms) / 1000
            ...
            if not ws_coverage_complete:
                return
            if accumulator.trade_count < MIN_TRADES_1M:
                return
            ...
            price_points = min(max(change_1m / 1.5 * 25, 0), 25)
            score += price_points
            if change_1m >= 0.5:
                reasons.append("✅ اختراق سعري سريع")

            if baseline_hours >= MIN_VOLUME_BASELINE_CANDLES and volume_spike_x > 0:
            ...
            baseline_text = (
                ...
                f"📏 متوسط الدقيقة: <b>{avg_hour_volume:,.0f} USDT</b> ({baseline_hours} شموع مغلقة)\n"
                if baseline_hours >= MIN_VOLUME_BASELINE_CANDLES and avg_hour_volume > 0
                else f"📈 ارتفاع الفوليوم: <b>جارٍ بناء المتوسط</b> "
                     f"({baseline_hours}/{MIN_VOLUME_BASELINE_CANDLES} شموع)\n"
            )
            ...
            message = (
                ...
                "━━━━━━━━━━━━━━━━━━\n"
                "💵 <b>Smart Volume — شمعة 1m المفتوحة</b>\n"
                ...
                f"{baseline_text}"
                "━━━━━━━━━━━━━━━━━━\n"
                "🟢 <b>ضغط الشراء — منذ افتتاح شمعة 1m</b>\n"
                ...
                f"• الصفقات منذ افتتاح شمعة 1m: <b>{accumulator.trade_count:,}</b>\n"
...
async def main():
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        if STARTUP_MESSAGE:
            await telegram.send(
                "✅ <b>Hype Coins Bot V13 — 1M Open Candle</b>\n"
                f"المنصات: <b>{', '.join(sorted(EXCHANGES))}</b>\n"
                f"نافذة WebSocket الاحتياطية: <b>{WINDOW_SECONDS} ثانية</b>\n"
                f"أقل تغير لشمعة 1m المفتوحة: <b>{MIN_CHANGE_1M}%</b>\n"
                f"أقل فوليوم لشمعة 1m المفتوحة: <b>{MIN_VOLUME_1M_USDT:,.0f} USDT</b>\n"
                f"أقل نسبة شراء: <b>{MIN_BUY_PERCENT}%</b>\n"
                f"متوسط الفوليوم: <b>آخر {VOLUME_BASELINE_CANDLES} شمعة 1m مغلقة</b>\n"
                f"أقل متوسط ساعة: <b>{MIN_AVERAGE_VOLUME_USDT:,.0f} USDT</b>\n"
                f"أقل Volume Spike فعلي: <b>{MIN_VOLUME_SPIKE:.2f}x</b>\n"
                f"أقل عدد صفقات في شمعة 1m: <b>{MIN_TRADES_1M}</b>"
            )
