...
class Detector:
    ...
    def official_candle_stats(candles):
        ...
        return {
            ...
            "volume_spike": pace_spike,
            "volume_change": pace_change,
    ...
    async def add_trade(self, trade: Trade) -> None:
        ...
        async with self.lock:
            ...
            trades_1m = [t for t in window if t.timestamp_ms >= current_hour_start_ms]
            ws_total = sum(t.quote_value for t in trades_1m)
            buys = sum(t.quote_value for t in trades_1m if t.side == "buy")
            buy_pct = (buys / ws_total * 100) if ws_total > 0 else 0.0

            if len(trades_1m) < 2 or buy_pct < MIN_BUY_PERCENT:
                return
            ...
            change_1m = official["change_1h"]
            change_5m = official["change_4h"]
            change_15m = official["change_12h"]
            ...
            message = (
                ...
                "⚡ <b>حركة السعر</b>\n"
                f"• 1h: <b>{change_1m:+.2f}%</b>\n"
                f"• 4h: <b>{change_5m:+.2f}%</b>\n"
                f"• 12h: <b>{change_15m:+.2f}%</b>\n"
                ...
                f"• المضاعف الخام الآن: <b>{raw_volume_spike_x:.2f}x</b>\n"
                f"{baseline_text}"
                "━━━━━━━━━━━━━━━━━━\n"
                ...
                f"• الصفقات منذ افتتاح الساعة: <b>{len(trades_1m):,}</b>\n"
