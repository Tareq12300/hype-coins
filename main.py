...
CANDLE_TIMEFRAME = os.getenv("CANDLE_TIMEFRAME", "15m").strip().lower()
...
class Detector:
    ...
    async def fetch_official_candles(self, exchange: str, symbol: str):
        ...
        if CANDLE_TIMEFRAME != "15m":
            raise ValueError("This version supports CANDLE_TIMEFRAME=15m only")
        ...
        try:
            if exchange == "gateio":
                ...
                params = {"currency_pair": symbol, "interval": "15m", "limit": CANDLE_LIMIT}
                ...
                params = {"symbol": symbol, "type": "15min"}
                async with session.get(url, params=params) as r:
                    r.raise_for_status()
                ...
                params = {"symbol": symbol, "granularity": "15min", "limit": str(CANDLE_LIMIT)}
                async with session.get(url, params=params) as r:
                    r.raise_for_status()
                    payload = await r.json()
            ...
            if cache_key not in self.invalid_candle_symbols:
                log.warning("%s %s official 15m candles failed: %s", exchange, symbol, exc)
            return []
    ...
    def official_candle_stats(candles):
        ...
        current_candle_start = (now_ms // 900_000) * 900_000
        current = None
        completed = []
        ...
        elapsed_fraction = min(elapsed_seconds / 900, 1.0)
        ...
        return {
            ...
            "change_15m": ((current["close"] / current["open"]) - 1) * 100 if current["open"] else 0.0,
            "change_1h": momentum(4),
            "change_4h": momentum(16),
        }
    ...
    async def add_trade(self, trade: Trade) -> None:
        ...
        async with self.lock:
            ...
            current_candle_start_ms = (now_ms // 900_000) * 900_000
            ...
            if not ws_coverage_complete:
                return
            if accumulator.trade_count < MIN_TRADES_15M:
                return
            ...
            price_points = min(max(change_15m / 3.0 * 25, 0), 25)
            score += price_points
            if change_15m >= 1.0:
            ...
            baseline_text = (
                ...
                f"📏 متوسط شمعة 15m: <b>{avg_hour_volume:,.0f} USDT</b> ({baseline_hours} شموع مغلقة)\n"
            ...
            message = (
                ...
                "━━━━━━━━━━━━━━━━━━\n"
                "💵 <b>Smart Volume — شمعة 15m المفتوحة</b>\n"
                ...
                f"{baseline_text}"
                "━━━━━━━━━━━━━━━━━━\n"
                "🟢 <b>ضغط الشراء — منذ افتتاح شمعة 15m</b>\n"
                ...
                f"• الصفقات منذ افتتاح شمعة 15m: <b>{accumulator.trade_count:,}</b>\n"
...
async def main():
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        if STARTUP_MESSAGE:
            await telegram.send(
                "✅ <b>Hype Coins Bot V11 — 15M Open Candle</b>\n"
                f"المنصات: <b>{', '.join(sorted(EXCHANGES))}</b>\n"
                f"نافذة WebSocket الاحتياطية: <b>{WINDOW_SECONDS} ثانية</b>\n"
                f"أقل تغير لشمعة 15m المفتوحة: <b>{MIN_CHANGE_15M}%</b>\n"
                f"أقل فوليوم لشمعة 15m المفتوحة: <b>{MIN_VOLUME_15M_USDT:,.0f} USDT</b>\n"
                f"أقل نسبة شراء: <b>{MIN_BUY_PERCENT}%</b>\n"
                f"متوسط الفوليوم: <b>آخر {VOLUME_BASELINE_CANDLES} شمعة 15m مغلقة</b>\n"
                f"أقل متوسط ساعة: <b>{MIN_AVERAGE_VOLUME_USDT:,.0f} USDT</b>\n"
                f"أقل Volume Spike فعلي: <b>{MIN_VOLUME_SPIKE:.2f}x</b>\n"
                f"أقل عدد صفقات في شمعة 15m: <b>{MIN_TRADES_15M}</b>"
            )
