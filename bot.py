...
def calculate_signal_score(
    *,
    ...
    Score from 0 to 100:
      MACD above zero             10
      MACD above signal           10
      MACD rising                 10
      Histogram positive          10
      Histogram rising            10
      Stoch RSI bullish cross     15
      Stoch RSI K rising          10
      Volume above average        10
      Strong volume ratio         10
      Price change in range        5
    """
    score = 0
    details: List[str] = []

    checks = [
        (macd > 0, 10, "MACD فوق الصفر"),
        (macd > macd_signal, 10, "MACD فوق Signal"),
        (macd > macd_prev, 10, "MACD صاعد"),
        (histogram > 0, 10, "Histogram موجب"),
        (histogram > histogram_prev, 10, "Histogram صاعد"),
        (
            stoch_k_prev <= stoch_d_prev and stoch_k > stoch_d,
            15,
            "تقاطع Stoch RSI صاعد",
        ),
        (stoch_k > stoch_k_prev, 10, "Stoch RSI صاعد"),
        (
            average_volume_usdt > 0 and current_volume_usdt > average_volume_usdt,
            10,
            "حجم الشمعة فوق المتوسط",
        ),
        (volume_ratio >= max(1.0, MIN_VOLUME_RATIO), 10, "Volume Ratio قوي"),
        (
            change >= MIN_PRICE_CHANGE_4H
            and (MAX_PRICE_CHANGE_4H <= 0 or change <= MAX_PRICE_CHANGE_4H),
            5,
            "تغير السعر ضمن النطاق",
        ),
    ]
...
def build_signal(row: pd.Series, universe: Dict[str, CoinMeta]) -> Optional[Signal]:
    ...
    score, score_details = calculate_signal_score(
        change=change,
        macd=macd,
        ...
        current_volume_usdt=current_volume_usdt,
...
def signal_message(signal: Signal) -> str:
    ...
    return (
        ...
        f"⭐ قوة الإشارة: <b>{signal.score}/100</b>\n\n"
        "📊 <b>MACD</b>\n"
...
def scan_once() -> Tuple[int, int, int]:
    ...
    candidates.sort(
        key=lambda s: (s.score, s.volume_ratio, s.candle_volume_usdt),
        reverse=True,
    )
...
def validate_environment() -> None:
    ...
    if not 0 <= MIN_SIGNAL_SCORE <= 100:
        raise ValueError("MIN_SIGNAL_SCORE يجب أن يكون بين 0 و100")
