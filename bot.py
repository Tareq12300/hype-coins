...
BOT_VERSION = "2026-07-13-multi-timeframe-1h-2h-v1"
...
TIMEFRAMES = [x.lower() for x in env_list("TIMEFRAMES", "1h,2h")]
...
logger = logging.getLogger("ta-stoch-macd-1h-2h-bot")
...
class BotRunner:
    ...
    async def analyze_symbol(
        ...
        async with self.sem:
            try:
                ...
                if result:
                    return exchange_id, symbol, timeframe, result
    ...
    def build_message(self, exchange_id: str, symbol: str, timeframe: str, r: Dict[str, float]) -> str:
...
💰 فوليوم شمعة {timeframe.upper()} الحالية:
...
""".strip()
    ...
    async def scan_exchange(self, exchange_id: str) -> int:
        ...
        try:
            ...
            tasks = [
                self.analyze_symbol(exchange, exchange_id, symbol, timeframe)
            ...
            for fut in asyncio.as_completed(tasks):
                ...
                if not result:
                    continue
                ex_id, symbol, timeframe, data = result
                ...
                if not self.cooldown_ok(key):
                    continue
                await self.send(self.build_message(ex_id, symbol, timeframe, data))
                self.mark_sent(key)
                sent_count += 1
    ...
    async def run(self):
        ...
        if not TIMEFRAMES:
            raise RuntimeError("TIMEFRAMES is empty")
        ...
        if invalid_timeframes:
            raise RuntimeError(
                ...
                "This version is configured for 1h and 2h."
            )
        logger.info("Running bot version: %s", BOT_VERSION)
        await self.send(
            f"✅ Bot started: {BOT_VERSION}\n"
            f"Stoch RSI + MACD + {', '.join(tf.upper() for tf in TIMEFRAMES)} volume + stock-token filter"
        )
        while True:
            try:
